...
namespace DA_TendonToolsWpf
{
    public class Commands
    {
        ...
        /// <summary>
        /// 显示钢束张拉量，本命令仅用于计算，通过本命令设置的参数不对图形数据库中存储的内容进行更新；
        /// 如果希望更新总体参数或钢束张拉方式，应通过DA_TendonTable或DA_TendonAnnotation进行。
        /// </summary>
        ...
        public void ShowTendonInfo()
        {
            ...
            using (Transaction trans = db.TransactionManager.StartTransaction())
            {
                ...
                {
                    string tdName = (string)td.ObjectId.GetXrecord("DA_Tendons")[0].Value;
                    info += "钢束名称：" + tdName + ";";
                    string tdStyle = (string)td.ObjectId.GetXrecord("DA_Tendons")[1].Value;
                    info += "\n钢束规格：" + tdStyle + ";";
                    Int16 tdNum = (Int16)td.ObjectId.GetXrecord("DA_Tendons")[2].Value;
                    info += "\n钢束根数：" + tdNum.ToString("F0") + ";";
                    double tdPipeDia = (double)td.ObjectId.GetXrecord("DA_Tendons")[3].Value;
                    info += "\n管道直径：" + tdPipeDia.ToString("F0") + " mm;";
                    int tdDrawStyle = (Int16)td.ObjectId.GetXrecord("DA_Tendons")[4].Value;
                    switch (tdDrawStyle)
                    {
                        case -1://左侧张拉
                            info += "\n张拉方式：" + "左侧张拉" + ";";
